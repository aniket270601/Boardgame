...
In CD Part, Terraform (main.tf) will provision the application server (on which application will be hosted). It will also provision amazon application load balancer. Ansible (appserver.yml) will make configutation changes and install required dependencies on application server to run the application. The provisioned application server will then be registered as a prometheus target via ansible and then Grafana will pull the metrics from prometheus data source.
